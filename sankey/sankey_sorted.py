...
#the difference between state1.py and this code is from line 155 to line 170 and from line 301 -302
...
def node_trace():
 ...
 #determine the value of the y axis as result[3]
    alen,lens,yxis=[],[],[]
    for i in range(0,20):
        alen.append(sum(vr[i*5:(i+1)*5]))
    for i in range(20,25):
        alen.append(vr[55+i]+vr[60+i]+vr[65+i]+vr[70+i]+vr[75+i])
    for i in range(0,25):
        lens.append((alen[i]/sum(alen[((i//5)*5):((i//5)*5+5)]))*0.8)
    print(lens)
    
    for i in range(0,25):
        a=i//5
        b=i%5
        yxis.append((sum(lens[(a*5):(a*5+b+1)])+b*0.05-lens[i]/2))
    #print(yxis)
    result.append(yxis)
...
def sankey():
    ...
    node = dict(
      ...
      x=[0.16,0.16,0.16,0.16,0.16,0.32,0.32,0.32,0.32,0.32,0.48,0.48,0.48,0.48,0.48,0.64,0.64,0.64,0.64,0.64,0.8,0.8,0.8,0.8,0.8],
      y=result[3],
    ...
    fig.write_image('./state1.svg',width=800,height=500)
    #fig.show()
