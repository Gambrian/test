...
def make_windows():
    ...
    for one_file in os.listdir(bedfile_dir):
        if "sorted" not in one_file:
            ...
            out_bed = os.path.join("/home/zhluo/Project/CRC/data_nazhang/step43_sankey/segment_200_bin",
                                   one_file + ".200bin")
...
def statistic_number():
    ...
    df = pd.read_csv(bin_file, sep="\t",
                     names=["chr", "start", "end", "ctrl_state", "week2_state", "week4_state", "week7_state",
                            "week10_state"])
    print(df)
    change_state_list = []
...
def node_trace():
    # total nodes = 65, 13*5
    lable = ["ctrl_" + "E_" + str(i) for i in range(1, 14)] + ["week2_" + "E_" + str(i) for i in range(1, 14)] + [
        "week4_" + "E_" + str(i) for i in range(1, 14)] + ["week7_" + "E_" + str(i) for i in range(1, 14)] + [
                "week10_" + "E_" + str(i) for i in range(1, 14)]
    lable_s = ["E" + str(i) for i in range(1, 14)] + ["E" + str(i) for i in range(1, 14)] + ["E" + str(i) for i in
                                                                                             range(1, 14)] + [
                  "E" + str(i) for i in range(1, 14)] + ["E" + str(i) for i in range(1, 14)]
    ...
    sr, tr, vr = [], [], []
    rowvalue = 0
    #divide thirteen structures into five states
    ...
    #get the value of every state in each stage
    ctrl_week2 = pd.read_csv("/home/zhluo/Project/CRC/data_nazhang/step43_sankey/trace/ctrl_week2.txt", sep="\t",
                             index_col=0)
    # print(ctrl_week2)
    ...
    week2_week4 = pd.read_csv("/home/zhluo/Project/CRC/data_nazhang/step43_sankey/trace/week2_week4.txt", sep="\t",
                              index_col=0)
    for i in range(1, 6):
        for j in range(1, 6):
            sr.append(i + 5)
    ...
    week4_week7 = pd.read_csv("/home/zhluo/Project/CRC/data_nazhang/step43_sankey/trace/week4_week7.txt", sep="\t",
                              index_col=0)
    for i in range(1, 6):
        for j in range(1, 6):
            sr.append(i + 5 + 5)
    ...
    week7_week10 = pd.read_csv("/home/zhluo/Project/CRC/data_nazhang/step43_sankey/trace/week7_week10.txt", sep="\t",
                               index_col=0)
    for i in range(1, 6):
        for j in range(1, 6):
            sr.append(i + 5 + 5 + 5)
            ...
            for sstate in sortlist["state" + str(i)]:
                for estate in sortlist["state" + str(j)]:
                    row = week7_week10[
                        (week7_week10["week7_state"] == sstate) & (week7_week10["week10_state"] == estate)]
                    if row.empty:
                        continue
                    rowvalue += int(row["count"])
            vr.append(rowvalue)
    ...
    #remove the unchanged parts that we don't care about
    m1, m2, m3, m4, m5 = [], [], [], [], []
    for i in range(1, 5):
...
def sankey():
    ...
    #let the line we don't care about turn gray
    for i in range(1, 6):
        for j in range(1, 21):
                if j % 5 != 4 and i != 4 :
                       linkcolor[j * 5 + i - 5 -1] = "rgba(190,190,190,0.5)"
    ...
    node = dict(
        ...
        label=["0", "state1", "state2", "state3", "state4", "state5", "state1", "state2", "state3", "state4", "state5",
    ...
    fig.update_layout(title_text="State4 Sankey Diagram", font_size=10)
